...
import type { CategoryType, ProductType } from '@/interfaces/index';
import { Logs, Notify } from '@/lib/index';
...
interface UCProductType {
  UpdateProduct?: { product_uid: string };
  CreateProduct?: { product_uid: string };
}
...
const Form = ({
  ...
  const SubmitProductDetails = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      if (title && price && product_description && !Loading) {
        ...
        await Request({
          ...
          variables: {
            [pid ? 'product_uid' : 'account_uid']: pid ?? account_uid,
            category_uid,
            title,
            price,
            discount,
          ...
          .then(({ UpdateProduct, CreateProduct }: UCProductType) => {
            const product_uid =
