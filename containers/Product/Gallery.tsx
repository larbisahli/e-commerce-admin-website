...
interface UIOType {
  UpdateImageOrder: { display_order: number };
}

const Gallery = ({ token, thumbnail, gallery, MutateProduct }: Props) => {
...
const ProductCard = memo(
  ({ token, index, url, image_uid, MutateProduct }: ProductCardProps) => {
    ...
    const SubmitSort = async (e: React.MouseEvent<HTMLButtonElement>) => {
      e.preventDefault();
      ...
      if (order > 0) {
        ...
        await Request({
          ...
          .then(({ UpdateImageOrder }: UIOType) => {
            ...
            const NotifyBoolean = typeof display_order === 'number';
            Notify(message, NotifyBoolean);
          })
          .catch(({ response }) => {
            const ErrorMessage =
