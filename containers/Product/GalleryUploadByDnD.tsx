...
const GalleryUploadByDnD = ({ token, MutateProduct }: Props) => {
  ...
  const [ThumbnailImage, setThumbnailImage] = useState<ImageListType>([]);

  const onImageChange = (imageList: ImageListType) => {
    setImages(imageList as never[]);
  };

  const onThumbnailChange = (imageList: ImageListType) => {
    setThumbnailImage(imageList as never[]);
  };

  const Form_Data = (image: ImageType, index: number) => {
